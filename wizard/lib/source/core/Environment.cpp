...
{
   QMenu* file = wkfEnv.GetMainWindow()->GetMenuByName(u8"文件");
   if (nullptr != file)
   {
      PopulateRecents();

      QAction* open = new QAction(u8"打开项目...", file);
...
void wizard::Environment::PopulateRecents()
{
   ...
   if (file)
   {
      QMenu* recent = file->findChild<QMenu*>(u8"打开最近");
      if (!recent)
      {
         recent = new QMenu(u8"打开最近", file);
         recent->setObjectName(u8"打开最近");
      ...
      if (rlist.isEmpty())
      {
         auto* action = recent->addAction(u8"(没有最近的文件)");
...
bool wizard::Environment::LoadScenario(const QStringList& aFiles)
{
   ...
   if (!projectPtr)
   {
      ...
      for (auto&& it : aFiles)
      {
         ...
         {
            QString message = u8"找不到文件： " + QString::fromStdString(path.GetSystemPath()) +
                              u8".\n选择“取消添加”以从您的最近列表中移除该项，或者选择“确定”以创建该项.";
            ...
            box->button(QMessageBox::Discard)->setText(u8"移除");
            int b = box->exec();
            delete box;
...
void wizard::Environment::FileOpen()
{
   ...
   if ((nullptr == sender) || (sender->text() == u8"打开项目..."))
   {
      QStringList files =
