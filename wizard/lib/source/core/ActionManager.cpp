...
{
   QMenu* editMenu = wkfEnv.GetMainWindow()->FindOrCreateMenu(u8"编辑", u8"视图");
   ...
   mUndo = new wkf::Action(QIcon::fromTheme("undo"), u8"撤销", qApp, QKeySequence("Ctrl+Z"), Qt::ApplicationShortcut);
   QObject::connect(mUndo, &QAction::triggered, [this]() { Undo(); });
   mRedo = new wkf::Action(QIcon::fromTheme("redo"), u8"恢复", qApp, QKeySequence("Ctrl+Shift+Z"), Qt::ApplicationShortcut);
   QObject::connect(mRedo, &QAction::triggered, [this]() { Redo(); });

   mCut = new wkf::Action(QIcon::fromTheme("scissors"), QString(u8"剪切"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+X"));
   QObject::connect(mCut, &QAction::triggered, [this]() { Cut(); });
   mCopy = new wkf::Action(QIcon::fromTheme("copy"), QString(u8"复制"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+C"));
   QObject::connect(mCopy, &QAction::triggered, [this]() { Copy(); });
   mPaste = new wkf::Action(QIcon::fromTheme("paste"), QString(u8"粘贴"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+V"));
   QObject::connect(mPaste, &QAction::triggered, [this]() { Paste(); });
   mSelectAll = new wkf::Action(QString(u8"全选"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+A"));
   QObject::connect(mSelectAll, &QAction::triggered, [this]() { SelectAll(); });

   mFind =
      new wkf::Action(QIcon::fromTheme("find_text"), QString(u8"查找..."), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+F"));
   QObject::connect(mFind, &QAction::triggered, [this]() { Find(); });
   mFindNext = new wkf::Action(QString(u8"查找下一个"), wkfEnv.GetMainWindow(), QKeySequence("F3"));
   QObject::connect(mFindNext, &QAction::triggered, [this]() { FindNext(); });
   mFindPrevious = new wkf::Action(QString(u8"查找上一个 "), wkfEnv.GetMainWindow(), QKeySequence("Shift+F3"));
   ...
   mReplace = new wkf::Action(QString(u8"替换..."), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+H"));
   QObject::connect(mReplace, &QAction::triggered, [this]() { Replace(); });

   mGoToLine = new wkf::Action(QString(u8"跳转到行..."), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+G"));
   QObject::connect(mGoToLine, &QAction::triggered, [this]() { GoToLine(); });

   mAutoComplete =
      new wkf::Action(QIcon::fromTheme("autocomplete"), u8"自动完成", wkfEnv.GetMainWindow(), QKeySequence("Ctrl+Space"));
   QObject::connect(mAutoComplete, &QAction::triggered, [this]() { Autocomplete(); });
   mCommentSelection =
      new wkf::Action(QString(u8"注释选择"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+K,Ctrl+C"));
   QObject::connect(mCommentSelection, &QAction::triggered, [this]() { CommentSelection(); });
   mUncommentSelection =
      new wkf::Action(QString(u8"取消注释选择"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+K,Ctrl+U"));
   QObject::connect(mUncommentSelection, &QAction::triggered, [this]() { UncommentSelection(); });

   mToggleComment = new wkf::Action(QString(u8"切换选择注释"), wkfEnv.GetMainWindow(), QKeySequence("#"));
   ...
   mCloseCurrent = new wkf::Action(QString(u8"关闭当前标签页"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+W"));
   QObject::connect(mCloseCurrent, &QAction::triggered, [this]() { CloseCurrentTab(); });

   mDelCurrentLine = new wkf::Action(QString(u8"删除当前行"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+Shift+K"));
...
{
   QMenu*          fileMenu   = wkfEnv.GetMainWindow()->FindOrCreateMenu(u8"文件");
   QList<QAction*> actions    = fileMenu->actions();
   wkf::Action*    newProject = new wkf::Action(QString(u8"新建项目..."), wkfEnv.GetMainWindow());
   QObject::connect(newProject, &QAction::triggered, [this]() { NewProject(); });
   mNewFile = new wkf::Action(QString(u8"新建文件..."), wkfEnv.GetMainWindow());
   QObject::connect(mNewFile, &QAction::triggered, [this]() { NewFile(); });
   wkf::Action* openFile = new wkf::Action(QString(u8"打开文件..."), wkfEnv.GetMainWindow());
   QObject::connect(openFile, &QAction::triggered, [this]() { OpenFile(); });
   mCloseProject = new wkf::Action(QString(u8"关闭项目"), wkfEnv.GetMainWindow());
   QObject::connect(mCloseProject, &QAction::triggered, [this]() { CloseProject(); });
   mSaveProject = new wkf::Action(QString(u8"保存项目"), wkfEnv.GetMainWindow());
   QObject::connect(mSaveProject, &QAction::triggered, [this]() { SaveProject(); });
   mExportProject = new wkf::Action(QString(u8"导出项目..."), wkfEnv.GetMainWindow());
   QObject::connect(mExportProject, &QAction::triggered, [this]() { ExportProject(); });

   mFileSave =
      new wkf::Action(QIcon::fromTheme("disk"), QString(u8"保存文件"), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+S"));
   QObject::connect(mFileSave, &QAction::triggered, [this]() { SaveCurrent(); });
   mFileSaveAs = new wkf::Action(QString(u8"另存为..."), wkfEnv.GetMainWindow());
   QObject::connect(mFileSaveAs, &QAction::triggered, [this]() { SaveCurrentAs(); });
   mFileSaveAll = new wkf::Action(QIcon::fromTheme("disks"), u8"保存全部", wkfEnv.GetMainWindow(), QKeySequence());
   QObject::connect(mFileSaveAll, &QAction::triggered, [this]() { SaveAll(); });
   mFilePrint = new wkf::Action(QString(u8"&打印文件..."), wkfEnv.GetMainWindow(), QKeySequence("Ctrl+P"));
...
{
   QMenu* sub = wkfEnv.GetMainWindow()->FindOrCreateViewSubMenu(u8"编辑");

   mCloseAll = new wkf::Action(QString(u8"关闭全部"), sub);
   QObject::connect(mCloseAll, &QAction::triggered, [this]() { CloseAll(); });
   mCloseNonStartup = new wkf::Action(QString(u8"除启动项全部关闭"), sub);
   QObject::connect(mCloseNonStartup, &QAction::triggered, [this]() { CloseAllButStartup(); });
   mCloseNonIncluded = new wkf::Action(QString(u8"除包含项全部关闭"), sub);
   QObject::connect(mCloseNonIncluded, &QAction::triggered, [this]() { CloseAllButIncluded(); });
   mCloseInactive = new wkf::Action(QString(u8"关闭非活动窗口"), sub);
...
{
   QMenu* viewMenu = wkfEnv.GetMainWindow()->FindOrCreateMenu(u8"项目");
   mAddDirectory   = new wkf::Action(QString(u8"将目录添加到项目..."), wkfEnv.GetMainWindow());
   QObject::connect(mAddDirectory, &QAction::triggered, [this]() { AddDirectory(); });
   mChangeHistory = new wkf::Action(QIcon::fromTheme("archive"), u8"变更历史...", wkfEnv.GetMainWindow());
   QObject::connect(mChangeHistory, &QAction::triggered, [this]() { ChangeHistory(); });
   mReparse = new wkf::Action(QIcon::fromTheme("repeat"), u8"重新解析", wkfEnv.GetMainWindow());
   QObject::connect(mReparse, &QAction::triggered, [this]() { Reparse(); });
   mProjectSettings = new wkf::Action(QString(u8"设置..."), wkfEnv.GetMainWindow());
...
{
   QMenu* helpMenu = wkfEnv.GetMainWindow()->FindOrCreateMenu(u8"帮助");

   mHelpCommandDocumentation =
      new wkf::Action(QString(u8"命令文档"), wkfEnv.GetMainWindow(), QKeySequence("F1"));
...
void wizard::ActionManager::BuildUpTextSourceContextMenu(QMenu& aMenu, TextSource* aSource, int aMainFile)
{
   ...
   {
      QAction* fileSave = new QAction(QIcon::fromTheme("disk"), u8"保存文件", &aMenu);
   ...
   aMenu.addAction(findInclude);
   QAction* renameFile = new QAction(QString(u8"重命名..."), &aMenu);
