...
{
    std::string text; // 用于存储转换后的字符串，支持多字节字符（如UTF-8编码的中文字符）

    // 预先分配足够的空间以减少内存重新分配的次数
    // 由于每个 QString 字符可能转换为多个 UTF-8 字节（特别是中文字符），
    // 这里简单预估为 aText.size() * 4（UTF-8 中一个字符最多占用4个字节）
    text.reserve(aText.size() * 4);

    // 遍历输入字符串中的每一个字符
    for (int i = 0; i < aText.size(); ++i)
    {
        QChar c = aText[i];           // 获取当前位置的 QChar 字符

        // 首先尝试将 QChar 转换为 Latin-1 (ASCII) 字符
        char ascii = c.toLatin1();

        if (ascii != '\0') // 如果转换成功（即字符在 ASCII 范围内）
        {
            // 特殊处理段落分隔符 U+2029，因为 toLatin1() 无法处理它
            if (c.unicode() == 0x2029)
            {
                text += '\n'; // 将段落分隔符转换为换行符
            }
            ...
            {
                // 其他 ASCII 字符直接添加到结果字符串中
                text += ascii;
            }
        }
        else
        {
                // 中文字符（U+4E00 到 U+9FFF）被保留为 UTF-8 编码
            if (c.unicode() >= 0x4E00 && c.unicode() <= 0x9FFF)
            {
                QString singleChar;
                singleChar.append(c);
                QByteArray utf8Bytes = singleChar.toUtf8();
                text.append(utf8Bytes.constData(), utf8Bytes.size());
            }
             else{
                text += ' ';
    ...
    return text; // 返回转换后的字符串，包含 ASCII 及 UTF-8 编码的字符
}
