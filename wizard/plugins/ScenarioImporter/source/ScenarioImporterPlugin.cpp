...
WKF_PLUGIN_DEFINE_SYMBOLS(ScenarioImporter::Plugin, u8"场景导入器", u8"提供了从 CSV 文件导入场景的功能", "wizard")
...
void ScenarioImporter::Plugin::LoadPlugin()
{
   ...
   fileWidget->setTitle(u8"文件");
   QFontMetrics fontMetric(fileWidget->font());
   int fileButtonWidth = fontMetric.width(u8"选择输出文件夹") + 10;

   // Open file button and text line
   gui.openFileButton = new QPushButton(u8"打开文件");
   ...
   gui.browseOutputDirectoryButton = new QPushButton(u8"选择输出文件夹");
   ...
   gui.loadImporterButton = new QPushButton(u8"加载导入器");
   ...
   gui.saveImporterButton = new QPushButton(u8"保存导入器");
   ...
   gui.saveOutputButton = new QPushButton(u8"保存输出");
   ...
   stagesWidget->setTitle(u8"处理阶段                  自动");

   gui.stageButtons[static_cast<int>(Stage::PreProcess)] = new QPushButton(u8"预处理");
   gui.stageButtons[static_cast<int>(Stage::Filter)] = new QPushButton(u8"过滤");
   gui.stageButtons[static_cast<int>(Stage::Parse)] = new QPushButton(u8"解析器");
   gui.stageButtons[static_cast<int>(Stage::Template)] = new QPushButton(u8"模板");
   gui.stageButtons[static_cast<int>(Stage::Preview)] = new QPushButton(u8"预览");
   ...
   gui.configureViewWidget->setTitle(u8"配置");
   ...
   gui.selectPreProcessorButton = new QPushButton(u8"预处理器程序");
   ...
   for (int stage = 0; stage < gui.configureViewLayout->count(); stage++)
   {
      ...
      gui.manualProcessButtons[stage]->setText(u8"处理");
   ...
   gui.inputViewWidget->addTab(gui.preProcessInputWidget, u8"预处理器输入");

   // Filter input widget
   gui.filterInputWidget = new QPlainTextEdit;
   gui.filterInputWidget->setReadOnly(true);
   gui.inputViewWidget->addTab(gui.filterInputWidget, u8"过滤器输入");

   // Parser input widget
   gui.parserInputWidget = new QPlainTextEdit;
   gui.parserInputWidget->setReadOnly(true);
   gui.inputViewWidget->addTab(gui.parserInputWidget, u8"解析器输入");
   ...
   gui.inputViewWidget->addTab(gui.templateInputWidget, u8"模板输入");
   ...
   gui.outputViewWidget->addTab(gui.preProcessOutputWidget, u8"预处理器输出");

   // Filter output widget
   gui.filterOutputWidget = new QPlainTextEdit;
   gui.filterOutputWidget->setReadOnly(true);
   gui.outputViewWidget->addTab(gui.filterOutputWidget, u8"过滤器输出");
   ...
   gui.outputViewWidget->addTab(gui.parserOutputWidget, u8"解析器输出");
   ...
   gui.outputViewWidget->addTab(gui.templateOutputWidget, u8"模板输出");

   // Preview output widget
   gui.previewOutputTabWidget = new QTabWidget;
   gui.outputViewWidget->addTab(gui.previewOutputTabWidget, u8"预览输出");
   ...
   gui.cancelButton = new QPushButton(u8"取消");
