...
WKF_PLUGIN_DEFINE_SYMBOLS(DemoBrowser::Plugin, u8"演示浏览器", u8"提供演示和测试场景的访问权限", "wizard")
   ...
   : wizard::Plugin(aPluginName, aUniqueId)
{
   ...
   QMenu*       help   = mainWindowPtr->FindOrCreateMenu(u8"帮助");
   wkf::Action* action = new wkf::Action(QString(u8"演示浏览器"), mainWindowPtr);
...
{
   QPushButton*               demoBrowserButtonPtr = new QPushButton(u8"演示浏览器");
