...
WKF_PLUGIN_DEFINE_SYMBOLS(VideoCapture::Plugin,
                          u8"视频捕获 ",
                          u8"视频捕获插件会在“工具”菜单中添加相关选项，以便进行屏幕截图和捕获操作 "
                          u8"来自该应用程序的视频.",
                          "all")
   ...
   , mFrameDeficit(0.0)
{
   ...
   {
      mVideoAction =
         new wkf::Action(QIcon::fromTheme("video_camera"), u8"录制视频", mainWindowPtr, QKeySequence("Ctrl+Shift+V"));
      connect(mVideoAction, &QAction::triggered, this, &Plugin::ToggleState);
      toolsMenuPtr->addAction(mVideoAction);
      mScreenAction =
         new wkf::Action(QIcon::fromTheme("camera"), u8"截图", mainWindowPtr, QKeySequence("Ctrl+Shift+S"));
