   ...
   , mDefaultChartIndex(0)
{
   ...
   mMap        = new QTreeWidgetItem(mUi.vectorTree->invisibleRootItem(), QStringList(u8"地图"));
   mMapChooser = new QComboBox(this);
   mUi.vectorTree->setItemWidget(mMap, 1, mMapChooser);

   QTreeWidgetItem* mapSettings = new QTreeWidgetItem(mUi.vectorTree->invisibleRootItem(), QStringList(u8"地图选项"));
   mIntensity                   = new QTreeWidgetItem(mapSettings, QStringList(u8"亮度"));
   ...
   mContrast       = new QTreeWidgetItem(mapSettings, QStringList(u8"对比度"));
   ...
   mSaturation       = new QTreeWidgetItem(mapSettings, QStringList(u8"饱和度"));
   ...
   QTreeWidgetItem* modelOptions = new QTreeWidgetItem(mUi.vectorTree->invisibleRootItem(), QStringList(u8"模型选项"));
   mTeamColor                    = new QTreeWidgetItem(modelOptions, QStringList(u8"显示队伍颜色"));
   mTeamColor->setCheckState(0, Qt::Unchecked);
   mTrueScale = new QTreeWidgetItem(modelOptions, QStringList(u8"真实比例"));
   mTrueScale->setCheckState(0, Qt::Unchecked);
   modelOptions->setExpanded(true);
   QTreeWidgetItem* modelScale = new QTreeWidgetItem(modelOptions, QStringList(u8"模型比例"));
   ...
   QTreeWidgetItem* labelSize = new QTreeWidgetItem(modelOptions, QStringList(u8"标签字体"));
   mLabelButton               = new UtQtFontButton(this);
   mLabelButton->SetWindowTitle(u8"选择平台标签字体");
   mUi.vectorTree->setItemWidget(labelSize, 1, mLabelButton);

   QTreeWidgetItem* overlays = new QTreeWidgetItem(mUi.vectorTree->invisibleRootItem(), QStringList(u8"叠加层"));
   mGrid                     = new QTreeWidgetItem(overlays, QStringList(u8"网格线"));
   mGrid->setCheckState(0, Qt::Unchecked);
   SetColor(mGrid, Qt::red);
   QTreeWidgetItem* gridDensity = new QTreeWidgetItem(mGrid, QStringList(u8"密度"));
   ...
   mElevationLines = new QTreeWidgetItem(overlays, QStringList(u8"等高线"));
   mElevationLines->setCheckState(0, Qt::Unchecked);

   mElevationSpacing = new QTreeWidgetItem(mElevationLines, QStringList(u8"间距 (m)"));
   SetValue(mElevationSpacing, 400, 10, 1000);
   mElevationMin = new QTreeWidgetItem(mElevationLines, QStringList(u8"最小 (m)"));
   SetValue(mElevationMin, -413, -413, 8850);
   mElevationMax = new QTreeWidgetItem(mElevationLines, QStringList(u8"最大 (m)"));
   SetValue(mElevationMax, 8850, -413, 8850);
   mElevationSource = new QTreeWidgetItem(mElevationLines, QStringList(u8"源头"));
   SetFilename(mElevationSource, "");

   mCoast = new QTreeWidgetItem(overlays, QStringList(u8"海岸线"));
   mCoast->setCheckState(0, Qt::Unchecked);
   SetColor(mCoast, Qt::red);
   mCountry = new QTreeWidgetItem(overlays, QStringList(u8"国界"));
   mCountry->setCheckState(0, Qt::Unchecked);
   SetColor(mCountry, Qt::red);
   mInternal = new QTreeWidgetItem(overlays, QStringList(u8"美国国内边界"));
   mInternal->setCheckState(0, Qt::Unchecked);
   SetColor(mInternal, Qt::red);
   mLakes = new QTreeWidgetItem(overlays, QStringList(u8"湖泊与河流"));
   mLakes->setCheckState(0, Qt::Unchecked);
   SetColor(mLakes, Qt::red);
   mRings = new QTreeWidgetItem(overlays, QStringList(u8"屏幕为中心的范围环"));
   mRings->setCheckState(0, Qt::Unchecked);
   SetColor(mRings, Qt::red);
   mTerminator = new QTreeWidgetItem(overlays, QStringList(u8"晨昏线"));
   mTerminator->setCheckState(0, Qt::Unchecked);
   SetColor(mTerminator, Qt::yellow);
   QTreeWidgetItem* terminatorWidth = new QTreeWidgetItem(mTerminator, QStringList(u8"线宽"));
                                       ...
                                       const QString&              aName)
   {
      ...
      QTreeWidgetItem* scale = new QTreeWidgetItem(aPoint, QStringList(u8"比例"));
      ...
      aDefaultIcon                = ut::qt::make_qt_ptr<QRadioButton>(u8"默认图标", groupFrame);
      aDefaultIcon->setChecked(true);
      aCustomIcon    = ut::qt::make_qt_ptr<QRadioButton>(u8"自定义图标", groupFrame);
   ...
   PopulateCelestialOptions(mSubSolarPoint, mSubSolarScale, mDefaultSunIcon, mCustomSunIcon, mSunImageSelector, u8"太阳直射点");
   PopulateCelestialOptions(mSubLunarPoint, mSubLunarScale, mDefaultMoonIcon, mCustomMoonIcon, mMoonImageSelector, u8"月亮直射点");

   mSolarPath = new QTreeWidgetItem(overlays, QStringList(u8"太阳轨迹"));
   mSolarPath->setCheckState(0, Qt::Unchecked);
   SetColor(mSolarPath, Qt::yellow);
   QTreeWidgetItem* solarPathWidth = new QTreeWidgetItem(mSolarPath, QStringList(u8"线宽"));
   ...
   mScale = new QTreeWidgetItem(overlays, QStringList(u8"比例"));
   mScale->setCheckState(0, Qt::Unchecked);
   mCompass = new QTreeWidgetItem(overlays, QStringList(u8"指南针"));
   mCompass->setCheckState(0, Qt::Unchecked);
   mLighting = new QTreeWidgetItem(mapSettings, QStringList(u8"光照"));
   mLighting->setCheckState(0, Qt::Unchecked);
   mECI = new QTreeWidgetItem(mapSettings, QStringList(u8"ECI 相机"));
   mECI->setCheckState(0, Qt::Unchecked);
   mCenterOnStart = new QTreeWidgetItem(mapSettings, QStringList(u8"开始时居中"));
   mCenterOnStart->setCheckState(0, Qt::Unchecked);
   mShowAltitudeAtCursor = new QTreeWidgetItem(mapSettings, QStringList(u8"显示光标处地形高度"));
   mShowAltitudeAtCursor->setCheckState(0, Qt::Unchecked);
   mZoomOnCursor = new QTreeWidgetItem(mapSettings, QStringList(u8"鼠标光标缩放"));
