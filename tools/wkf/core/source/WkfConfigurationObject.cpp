...
void wkf::ConfigurationObject::Initialize()
{
   ...
   // Find the File Menu
   QMenu* fileMenu = wkfEnv.GetMainWindow()->GetMenuByName(u8"文件");
   if (fileMenu != nullptr)
   {
      Action* saveAction = new Action(QIcon::fromTheme("save_settings"), u8"保存配置...", wkfEnv.GetMainWindow());
      connect(saveAction, &QAction::triggered, this, &ConfigurationObject::SaveActionHandler);

      Action* loadAction = new Action(QIcon::fromTheme("load_settings"), u8"加载配置...", wkfEnv.GetMainWindow());
      connect(loadAction, &QAction::triggered, this, &ConfigurationObject::LoadActionHandler);

      Action* importAction = new Action(u8"导入配置选项...", wkfEnv.GetMainWindow());
      connect(importAction, &QAction::triggered, this, &ConfigurationObject::ImportActionHandler);

      mLoadRecentMenu = new QMenu(u8"最近配置", fileMenu);
      ...
      Action* clearPlatformSettings = new Action(u8"清除平台选项", wkfEnv.GetMainWindow());
...
void wkf::ConfigurationObject::LoadActionHandler()
{
   ...
   QString filePath =
      wkf::getOpenFileName(nullptr, u8"加载配置", Environment::GetInstance().GetConfigurationsDir(), "(*.ini)");
   ...
   if (filePath != "")
   {
      ...
      {
         QMessageBox::warning(nullptr, u8"文件未找到", u8"配置文件未找到.");
      }
   }
}
...
void wkf::ConfigurationObject::ImportActionHandler()
{
   CheckForConfigDirectory();
   QString filePath =
      wkf::getOpenFileName(nullptr, u8"从配置中导入", Environment::GetInstance().GetConfigurationsDir(), "(*.ini)");
   ...
   if (!filePath.isEmpty())
   {
      ...
      if (info.exists())
      {
         ...
         QPushButton* checkAll   = new QPushButton(u8"全选", &selector);
         QPushButton* uncheckAll = new QPushButton(u8"取消全选", &selector);
...
void wkf::ConfigurationObject::PopulateMenu(QMenu* aMenu)
{
   ...
   if (aMenu->isEmpty())
   {
      auto* action = aMenu->addAction(u8"(没有最近配置)");
