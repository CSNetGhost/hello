   ...
   , mTypeName(aTypeName)
{
   ...
   mFullScreenActionPtr =
      new Action(QIcon::fromTheme("fullscreen"), u8"全屏", this, QKeySequence(QKeySequence::FullScreen));
...
void wkf::DockWidget::BuildContextMenuP(QMenu& aMenu, const QPoint& pos)
{
   ...
   {
      Action* action = new Action(QIcon::fromTheme("move"), u8"移动到新窗口", &aMenu);
      connect(action, &QAction::triggered, this, [this]() { setFloating(true); });
      aMenu.addAction(action);

      auto* hideAction = new QAction(QIcon::fromTheme("hide"), u8"隐藏", &aMenu);
