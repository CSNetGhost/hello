...
namespace
{
const QString cDOTMARKER  = u8"点标记";
const QString cXMARKER    = u8"X 标记";
const QString cCUSTOMICON = u8"自定义图标";
const QString cIMAGE      = u8"图标";
} // namespace

wkf::EventMarkerPrefWidget::EventMarkerPrefWidget()
   : wkf::PrefWidgetT<wkf::EventMarkerPrefObject>()
{
   setWindowTitle(u8"事件标记");

   QFormLayout* form = new QFormLayout(this);
   setLayout(form);
   auto* label     = new QLabel(u8"此页面上的设置会在所有 AFSIM 应用程序之间共享", this);
   ...
   mItemMap["REMOVED"] = new QTreeWidgetItem(mTreeWidget->invisibleRootItem(), QStringList(u8"平台移除"));
   PopulateOptions(mItemMap["REMOVED"]);

   mItemMap["DAMAGED"] = new QTreeWidgetItem(mTreeWidget->invisibleRootItem(), QStringList(u8"平台受损"));
   PopulateOptions(mItemMap["DAMAGED"]);

   mItemMap["WEAPON_HIT"] = new QTreeWidgetItem(mTreeWidget->invisibleRootItem(), QStringList(u8"武器命中"));
   PopulateOptions(mItemMap["WEAPON_HIT"]);

   mItemMap["WEAPON_MISSED"] = new QTreeWidgetItem(mTreeWidget->invisibleRootItem(), QStringList(u8"武器未命中"));
