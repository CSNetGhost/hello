...
{
    mRangeRingOption = RegisterOption(nullptr, u8"范围环", "", false, false, true)->type();
    mDecorationOption = RegisterOption(nullptr, u8"装饰", "", false, false, true)->type();
    QTreeWidgetItem* vectors = RegisterOptionGroup(nullptr, u8"向量");
    mBodyFrameOption = RegisterOption(vectors, u8"机体坐标系")->type();
    mVelocityOption = RegisterOption(vectors, u8"速度")->type();
    mEarthOption = RegisterOption(vectors, u8"星下点")->type();
    mSunOption = RegisterOption(vectors, u8"太阳")->type();
    mMoonOption = RegisterOption(vectors, u8"月亮")->type();
...
void Annotation::Plugin::BuildEntityContextMenu(QMenu* aMenu, wkf::Entity* aEntityPtr)
{
   if (mEnableFeatures)
   {
      ...
      if (isPOI || isPlatform)
      {
         ...
         {
            auto rrMenu = ut::qt::make_qt_ptr<QMenu>(QString(u8"范围环"), aMenu);
...
void Annotation::Plugin::BuildViewerContextMenu(QMenu* aMenu, vespa::VaViewer* aViewerPtr)
{
   if (aViewerPtr == vaEnv.GetStandardViewer() && mEnableFeatures)
   {
      ...
      if (!addMenu)
      {
         addMenu = aMenu->addMenu(u8"在指定位置添加");  // 将菜单名称翻译成中文
         addMenu->setObjectName("Add at Location");
         UtQtTranslucentMenu(*addMenu);
      }

      vespa::VaPosition p = vaEnv.GetCurrentCursorPosition();
      auto bullseye       = ut::qt::make_qt_ptr<QAction>(QIcon::fromTheme("bullseye"), QString(u8"靶心"), addMenu);
      addMenu->addAction(bullseye);
      auto pointOfInterest = ut::qt::make_qt_ptr<QAction>(QIcon::fromTheme("poi"), QString(u8"兴趣点"), addMenu);
      addMenu->addAction(pointOfInterest);
      auto rangeRing = ut::qt::make_qt_ptr<QAction>(QIcon::fromTheme("rangering"), QString(u8"距离环"), addMenu);
