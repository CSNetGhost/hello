   ...
   : warlock::PluginT<SimInterface>(aPluginName, aUniqueId)
{
   ...
   RegisterOption(parent, u8"航线");

   wkf::Action* headingAction = new wkf::Action(u8"转向", wkfEnv.GetMainWindow(), QKeySequence(Qt::Key_H));
   connect(headingAction, &QAction::triggered, this, [this]() { mShortcutPtr->StartHeading(); });
   mActions.push_back(headingAction);

   wkf::Action* gotoAction = new wkf::Action(u8"到某一位置", wkfEnv.GetMainWindow(), QKeySequence(Qt::Key_G));
