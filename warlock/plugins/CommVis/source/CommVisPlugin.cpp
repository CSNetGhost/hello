...
WKF_PLUGIN_DEFINE_SYMBOLS(WkCommVis::Plugin,
                          u8"通信可视化",
                          u8"The Comms Visualization plugin creates a dialog, under the Tools menu, that shows activity "
   ...
   : warlock::PluginT<SimInterface>(aPluginName, aUniqueId)
{
   ...
   if (toolsMenuPtr)
   {
      auto dlgActionPtr = ut::qt::make_qt_ptr<wkf::Action>(QIcon::fromTheme("star"), u8"通信可视化", mainWindowPtr);
