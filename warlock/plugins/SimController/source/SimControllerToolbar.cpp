...
#endif
{
   ...
   auto& enableToolbar =
      wkfEnv.GetPermissions().AddBool(u8"仿真控制器",
                                      u8"启用仿真控制",
   ...
   mPauseActionPtr = new wkf::Action(QIcon::fromTheme("pause"), u8"暂停", pauseResumeGroup, QKeySequence(Qt::Key_F5));
   connect(mPauseActionPtr,
           &QAction::triggered,
           [this] { mSimInterfacePtr->AddSimCommand(ut::make_unique<PauseCommand>(mSendDIS_PDUs)); });
   mResumeActionPtr = new wkf::Action(QIcon::fromTheme("play"), u8"恢复", pauseResumeGroup, QKeySequence(Qt::Key_F5));
   ...
   mTerminateActionPtr =
      new wkf::Action(QIcon::fromTheme("stop"), u8"终止", this, QKeySequence(Qt::SHIFT + Qt::Key_F5));
   connect(mTerminateActionPtr, &QAction::triggered, this, &Toolbar::HandleTerminateSimAction);

   mRestartActionPtr =
      new wkf::Action(QIcon::fromTheme("restart"), u8"重启", this, QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_F5));
   connect(mRestartActionPtr, &QAction::triggered, this, &Toolbar::HandleRestartSimAction);

   addSeparator();

   wkf::Action* clockRateActionPtr     = new wkf::Action(QIcon::fromTheme("fast_forward"), u8"设置时钟速率", this);
   wkf::Action* advanceToTimeActionPtr = new wkf::Action(QIcon::fromTheme("time"), u8"推进到指定时间", this);
...
void WkSimController::Toolbar::CreateThumbnailToolbar()
{
   ...
   if (mCreateThumbToolbar && !mThumbToolbarPtr)
   {
      ...
      mThumbToggleRunButtonPtr->setToolTip(paused ? u8"继续" : u8"暂停");
      ...
      termButtonPtr->setToolTip(u8"终止");
