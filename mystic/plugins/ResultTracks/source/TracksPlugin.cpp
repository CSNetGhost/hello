...
WKF_PLUGIN_DEFINE_SYMBOLS(RvTracks::Plugin, u8"轨迹", u8"在地图显示上展示轨迹信息.", "mystic")
...
RvTracks::Plugin::TrackItems::TrackItems(QTreeWidgetItem* aParentItem, int aTrackId, const QString& aPlatformName, bool aLocal)
{
   ...
   mPositionValid = new wkf::PlatformDataTreeItem(mParentItem, {u8"有效位置"});
   mLatitude      = new wkf::LatitudeTreeWidgetItem(mParentItem, u8"纬度");
   mLongitude     = new wkf::LongitudeTreeWidgetItem(mParentItem, u8"经度");
   mAltitude      = new wkf::AltitudeTreeWidgetItem(mParentItem, u8"海拔");
   mVelocityValid = new wkf::PlatformDataTreeItem(mParentItem, {"Velocity Valid"});
   mSpeed         = new wkf::SpeedTreeWidgetItem(mParentItem, u8"速度");
   mHeading       = new wkf::AngleTreeWidgetItem(mParentItem, u8"航向");
   mSide          = new wkf::PlatformDataTreeItem(mParentItem, {u8"阵营"});
   mType          = new wkf::PlatformDataTreeItem(mParentItem, {u8"类型"});
   mSpatialDomain = new wkf::PlatformDataTreeItem(mParentItem, {u8"空间域"});
